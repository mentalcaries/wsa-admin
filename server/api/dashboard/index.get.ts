...
import { getEventCount, getRecentEvents } from '../../utils/posthog';
...
export default defineEventHandler(async (event) => {
  try {
    ...
    const [
      ...
      supabase
        .from('subscriptions')
        .select('tier, reports_used', { count: 'exact', head: true })
        .eq('status', 'active')
        .or(
          `tier.eq.basic,reports_used.gte.${TIER_LIMITS.basic},tier.eq.professional,reports_used.gte.${TIER_LIMITS.professional},tier.eq.enterprise,reports_used.gte.${TIER_LIMITS.enterprise}`,
        ),
